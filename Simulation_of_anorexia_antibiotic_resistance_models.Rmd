...
```{r highIngestFig1, fig.height=4, fig.width=6, fig.cap="Probability of drug-resistance emerging when ingestion is high."}
...
# 
# pars <- c(theta=10, chi=0.1, alpha1=0.1, alpha2=0.08, eta1=1500, eta2=1500, epsilon1=20, epsilon2=20, delta1=0.1, delta2=0.1, mu=0.0001, D=0, tDrug=30)
# out <- stochastic_strain_competition_w_drugs(400, tstep=0.1, pars=pars, y0=c(R=unname(pars["theta"]/pars["chi"]), P1=10, P2=0), seed=13234)
# par(mfrow=c(1,3))
# plot(out[,c(1,2)], type='l')
# abline(h=pars["eta2"]*pars["delta2"]/(pars["epsilon2"]*pars["alpha2"]-pars["delta2"])) ## resource requirement for strain 2
# plot(out[,c(1,3)], type='l')
# plot(out[,c(1,4)], type='l')
# 
# pars <- c(theta=10, chi=0.1, alpha1=0.1, alpha2=0.08, eta1=1500, eta2=1500, epsilon1=20, epsilon2=20, delta1=0.1, delta2=0.1, mu=0.01, D=5, tDrug=80)
# out <- stochastic_strain_competition_w_drugs(200, tstep=0.1, pars=pars, y0=c(R=unname(pars["theta"]/pars["chi"]), P1=10, P2=0), seed=13234)
# par(mfrow=c(1,3))
# plot(out[,c(1,2)], type='l')
# abline(h=pars["eta2"]*pars["delta2"]/(pars["epsilon2"]*pars["alpha2"]-pars["delta2"])) ## resource requirement for strain 2
# plot(out[,c(1,3)], type='l')
# plot(out[,c(1,4)], type='l')
# 
# ## With this set of parameters, drug resistance emerges seldom
# pars <- c(theta=10, chi=0.1, alpha1=0.1, alpha2=0.09, eta1=1500, eta2=1500, epsilon1=20, epsilon2=20, delta1=0.1, delta2=0.1, mu=0.001, D=10, tDrug=150)
# out <- stochastic_strain_competition_w_drugs(500, tstep=0.1, pars=pars, y0=c(R=unname(pars["theta"]/pars["chi"]), P1=10, P2=0), seed=floor(runif(1,1,1e5)))
# par(mfrow=c(1,3))
# plot(out[,c(1,2)], type='l')
# abline(h=pars["eta2"]*pars["delta2"]/(pars["epsilon2"]*pars["alpha2"]-pars["delta2"])) ## resource requirement for strain 2
# plot(out[,c(1,3)], type='l')
...
if(!(file.exists("out10.RDS"))) {
  ...
  saveRDS(out10, "out10.RDS")
} else out10 <- readRDS("out10.RDS")
...
I propose a straightforward explanation for this: if resistance has emerged, whether dose is 5 or 20 is irrelevant - it's enough to give a big fitness advantage to the drug susceptible strain and thus to facilitate the evolution of resistance.

```{r highIngestFig2, fig.height=4, fig.width=6, fig.cap="Probability of drug-resistance emerging when ingestion is high."}
...
par(mar=c(3,5,0.5,0.5))
plot(seq(0,5,0.25), timeToExtinction, pch=21, bg=1, cex=1.5, xaxt='n', ylab="Time until susceptible extinction", cex.lab=1.5, cex.axis=1.65)
axis(1,tick=TRUE,labels=FALSE)
mtext(side=1, line=1.5, "Drug dose", cex=1.65)               
...
I can further validate this hypothesis by reducing the mutation rate and seeing whether that changes the overall pattern, making it less likely for resistance to emerge at higher doses.

```{r}
if(!(file.exists("out10_2.RDS"))) {
  seeds <- readRDS("seeds.RDS")
  lapply(seq(0,5,0.25), 
         function(drug) {
           print(drug)
  ...
  saveRDS(out10.2, "out10_2.RDS")
} else out10.2 <- readRDS("out10_2.RDS")

if(!(file.exists("out8.RDS"))) {  
  lapply(seq(0,5,0.25), 
         function(drug) {
           print(drug)
  ...
  saveRDS(out8, "out8.RDS")
} else out8 <- readRDS("out8.RDS")
  
if(!(file.exists("out8_2.RDS"))) {  
  lapply(seq(0,5,0.25), 
         function(drug) {
           print(drug)
  ...
  saveRDS(out8.2, "out8_2.RDS")
} else out8.2 <- readRDS("out8_2.RDS")

cutoff <- lapply(out10.2[[1]], function(o) tail(o[,4],10) %>% mean) %>% unlist %>% mean(.,na.rm=TRUE)

lapply(1:length(out10.2),
       function(i) (lapply(out10.2[[i]], function(o) (ifelse((!is.na(tail(o[,4],1))),tail(o[,4],10) %>% mean,0) > 2*cutoff)) %>% unlist %>% sum)/length(out10.2[[i]])) %>% unlist -> emergeProb10.2

par(mar=c(3,5,0.5,0.5))
plot(seq(0,5,0.25), emergeProb10.2, pch=21, bg=1, cex=1.5, xaxt='n', ylab="Probability of resistance", cex.lab=1.5, cex.axis=1.65)
axis(1,tick=TRUE,labels=FALSE)
mtext(side=1, line=1.5, "Drug dose", cex=1.65)
